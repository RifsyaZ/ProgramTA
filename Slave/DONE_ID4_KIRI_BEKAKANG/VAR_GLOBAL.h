...
#define DRIVE_RPWM PB6
#define DRIVE_LPWM PB7
...
// ==================== COMMAND TIMEOUT ====================
#define CMD_TIMEOUT_MS 300  // Commands dianggap sama jika dalam 300ms
...
// Last command tracking untuk mencegah reset
extern float lastRs485Angle;
extern float lastRs485Rpm;
extern unsigned long lastRs485CmdTime;

// Queue & Semaphore handles
extern QueueHandle_t steerCommandQueue;
extern QueueHandle_t driveCommandQueue;
extern QueueHandle_t rs485CommandQueue;
...
extern TaskHandle_t rs485ExecuteTaskHandle;

// Buffers
extern char inputBuffer[50];
extern int bufferIndex;
extern char rs485Buffer[100];
extern int rs485BufferIndex;
...
typedef struct {
  ...
  float rpm;
  bool stop_all;
  bool is_new_command;  // NEW: Flag untuk menandai command baru
} SwerveCommand_t;
...
float lastRs485Angle = -999.0;  // Nilai impossible sebagai initial
float lastRs485Rpm = -999.0;
unsigned long lastRs485CmdTime = 0;

// Queue & Semaphore
QueueHandle_t steerCommandQueue;
QueueHandle_t driveCommandQueue;
QueueHandle_t rs485CommandQueue;
...
TaskHandle_t rs485ExecuteTaskHandle;

// Buffers
char inputBuffer[50];
int bufferIndex = 0;
char rs485Buffer[100];
int rs485BufferIndex = 0;
