...
void drive_applyOutput() {
  ...
  if (drive_wheelRpmTarget_requested > 0) {
...
void drive_setTargetRpm(float rpm, bool forward) {
  // JANGAN RESET JIKA RPM SAMA (ini kunci untuk mencegah patah-patah)
  float newTargetRequested = forward ? rpm : -rpm;
  
  // Cek apakah target sama dengan yang sedang berjalan
  if (drive_motorRunning && driving_active && 
      newTargetRequested == drive_wheelRpmTarget_requested) {
    // Target sama, JANGAN reset PID atau restart motor!
    return;
  }
  
  drive_wheelRpmTarget_requested = newTargetRequested;
  ...
  // Hanya reset PID jika ini benar-benar command baru
  if (!drive_motorRunning) {
...
void drivingTask(void *pvParameters) {
  ...
  // Tracking untuk mencegah reset berulang
  static float last_angle = -999;
  static float last_rpm = -999;
  ...
  while(1) {
    ...
    if (xQueueReceive(driveCommandQueue, &receivedCmd, 0) == pdTRUE) {
      if (receivedCmd.stop_all) {
        ...
        // Cek apakah command benar-benar baru (angle atau rpm berubah)
        if (receivedCmd.angle != last_angle || receivedCmd.rpm != last_rpm) {
          last_angle = receivedCmd.angle;
          last_rpm = receivedCmd.rpm;
        ...
        // Jika angle dan rpm sama, IGNORE (jangan reset)
      }
    }
