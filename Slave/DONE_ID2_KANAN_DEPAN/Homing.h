...
#define HOMING_SENSOR_HIGH 1010
#define HOMING_SENSOR_LOW 290
...
int homing_checkPosition() {
  ...
  if (kiri <= HOMING_SENSOR_LOW && kanan >= HOMING_SENSOR_HIGH) {
    return ARAH_HOME;
  }
...
static void homing_setMotor(int arahPutar) {
  ...
  pidOutput = (HOMING_PID_KP * pidError) + 
              (HOMING_PID_KI * pidIntegral) + 
              (HOMING_PID_KD * derivative);
