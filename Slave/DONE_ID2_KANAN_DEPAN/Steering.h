...
void steer_setTargetWithSpeed(float angle, float speed) {
  ...
  float newTarget = constrain(steer_normalizeSwerveAngle(angle), STEER_MIN_ANGLE, STEER_MAX_ANGLE);
  
  // JANGAN SET ULANG JIKA TARGET SAMA (anti-reset)
  if (steering_active && newTarget == steerTargetAngle) {
    return;
  }
  
  steerRawTargetAngle = angle;
  steerTargetAngle = newTarget;
  ...
  while(1) {
    ...
    if (xQueueReceive(steerCommandQueue, &receivedCmd, 0) == pdTRUE) {
      if (receivedCmd.stop_all) {
        ...
        // Hanya set target jika angle berubah
        if (receivedCmd.angle != last_angle) {
    ...
    // Kirim sudut steering realtime ke Serial2
    Serial2.print("STEER:");
